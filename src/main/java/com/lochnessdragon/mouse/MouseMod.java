...
public class MouseMod {
	...
	public static class RegistryEvents {
		...
		public static void registerItem(final RegistryEvent.Register<Item> event) {
			...
			if (CheeseConfig.ENABLED.get()) {
				event.getRegistry().registerAll(
					ItemList.cheese = new Item(new Item.Properties().group(ItemGroup.FOOD).food(new Food.Builder().setAlwaysEdible().fastToEat().hunger(2).saturation(1.5f).effect(new EffectInstance(Effects.SPEED, 100, 3), 1f).build())).setRegistryName(location("cheese")),
					ItemList.cheese_block = new BlockItem(BlockList.cheese_block, new Item.Properties().group(ItemGroup.DECORATIONS)).setRegistryName(location("cheese_block"))
				);
			}
		...
		public static void registerBlock(final RegistryEvent.Register<Block> event) {
			...
			if (CheeseConfig.ENABLED.get()) {
				event.getRegistry().register(BlockList.cheese_block = new Block(Block.Properties.create(Material.CAKE).hardnessAndResistance(2.0f, 3.0f).sound(SoundType.CROP)).setRegistryName(location("cheese_block")));
			}
